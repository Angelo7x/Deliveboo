                ...
                <div class="card-body">
					<form action="{{route("admin.foods.update", $food["id"])}}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('image') is-invalid @enderror" name="image" id="image" placeholder="Inserisci l'url dell'immagine" value="{{old("image") ?? $food["image"]}}">
                            @error('image')
                              <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
